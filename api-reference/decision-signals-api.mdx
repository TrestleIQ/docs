...
<Warning>
  **Note:** Authentication can be provided either as an `x-api-key` header or as
  an `api_key` query parameter. One of the two is required. The header is
  recommended, since query strings are recorded in server access logs and
  browser history.
</Warning>
...
<ParamField query="api_key" type="string">
  Your Trestle API key, as an alternative to the `x-api-key` header. One of the
  two is required. The header is recommended. **Example:** `api_key=YOUR_API_KEY`
</ParamField>
...
<ParamField header="x-api-key" type="string">
  ...
  `api_key` query parameter. **Example:** `{{ apiKey }}`
</ParamField>
