---
title: "Real Contact API 2.0"
description: "Real Contact API verifies and grades phone numbers, emails, and addresses and delivers essential insights, including a phone activity score, line types, name matches for phone, email, and address, phone and email contact grades, and IP-based trust scoring."
...
<RequestExample>
    ```bash cURL
    curl --request GET \
      --url "https://api.trestleiq.com/2.0/real_contact?name=John%20Doe&phone=4259853735&email=john.doe@example.com" \
      --header "x-api-key: YOUR_API_KEY"
    ```

    ```javascript JavaScript
    const response = await fetch(
      "https://api.trestleiq.com/2.0/real_contact?name=John%20Doe&phone=4259853735&email=john.doe@example.com",
      {
        headers: {
    ...
    var response = await client.GetAsync("https://api.trestleiq.com/2.0/real_contact?name=John%20Doe&phone=4259853735&email=john.doe@example.com");
    ...
    func main() {
    	req, _ := http.NewRequest("GET", "https://api.trestleiq.com/2.0/real_contact?name=John%20Doe&phone=4259853735&email=john.doe@example.com", nil)
    ...
    ```php PHP
    <?php
    $curl = curl_init("https://api.trestleiq.com/2.0/real_contact?name=John%20Doe&phone=4259853735&email=john.doe@example.com");
...
<ResponseField name="ip_address" type="object or null">
  IP address enrichment and trust scoring results. Returned when an IP address is provided in the request.
  <Expandable title="ip_address object" defaultOpen>
    <ResponseField name="trust_score" type="integer or null">
      The IP Trust Score is a 0–100 score that indicates how reliable and trustworthy an interaction appears based on the user's IP address. It helps determine whether the lead/sign-up, based on the IP address, can be trusted or is potentially risky. It is calculated using a machine-learning model that evaluates multiple characteristics of the IP address and how it behaves over time.
    </ResponseField>
    <ResponseField name="ip_distance_to_address" type="number or null">
      The approximate distance (in miles) between the user's IP location and the provided address. This helps assess whether the user's location aligns with the provided contact details. This returns NULL values when confidence in the IP addresses' address accuracy is low.
    </ResponseField>
  </Expandable>
</ResponseField>
