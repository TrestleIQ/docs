...
| `403` | Forbidden | Missing or malformed x-api-key header, missing product or version access inactive, revoked or disabled API key |
...
```json
{
  ...
  "hint": [
    "Verify that the 'x-api-key' header is present and correctly formatted, with no extra spaces.",
    "Confirm that the API key has access to the requested product and product version.",
    "Confirm that the API key is active and has not been revoked or disabled."
  ]
}
```
...
|---|---|---|
| `AUTHENTICATION_FAILED` | `403` | API key is missing, malformed, invalid, revoked, disabled, expired, or not authorized for the requested product/version |
...
|---|---|---|---|
| Invalid API Key | `AUTHENTICATION_FAILED` | Key is incorrect or deactivated | Check for trailing spaces or character errors in the key |
| API Key Missing | `AUTHENTICATION_FAILED` | `x-api-key` header not included | Add the header to your request |
| API Key Disabled | `AUTHENTICATION_FAILED` | Key is inactive (portal issue) | Check wallet balance or contact Trestle support |
| No Product Access | `AUTHENTICATION_FAILED` | Key not enabled for this product/version | Enable the product in the portal or contact support |
| API Key Expired | `AUTHENTICATION_FAILED` | Key reached end-of-life (trial users) | Renew or upgrade in the portal |

A missing, incorrect, inactive, or expired `x-api-key`, or one not enabled for the requested product or version, returns `AUTHENTICATION_FAILED`.
